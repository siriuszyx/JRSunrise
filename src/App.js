...
import { Calendar, Train, MapPin, Clock, ExternalLink, AlertCircle } from 'lucide-react';
...
const App = () => {
    ...
    const destinationLogic = {
        ...
        "Osaka": ["Shizuoka", "Yokohama", "Tokyo"],
        "Sannomiya": ["Shizuoka", "Yokohama", "Tokyo"],
    ...
    // Separate valid trains from errors
    const separateResultsAndErrors = (results) => {
        const validTrains = [];
        const errors = [];
        ...
        results.forEach(result => {
            if (result.error) {
                ...
                validTrains.push(result);
            }
        });

        return { validTrains, errors };
    };
            ...
            <div className="max-w-4xl mx-auto">
                ...
                {Object.keys(searchResults).length > 0 && (
                    <div className="bg-white rounded-xl shadow-lg p-8">
                        ...
                        {Object.entries(searchResults).map(([date, results]) => {
                            const { validTrains, errors } = separateResultsAndErrors(results);
                            ...
                            return (
                                <div key={date} className="mb-8 last:mb-0">
                                        ...
                                        <div className="mb-4 space-y-2">
                                            {errors.map((error, index) => (
                                                <div key={index} className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-red-800 font-medium">{t.searchError}</p>
                                                        ...
                                                        {error.link && (
                                                            <a
                                                                ...
                                                                className="text-red-600 hover:text-red-800 text-sm underline inline-flex items-center gap-1 mt-1"
                                                            >
                                                                View Source
                                    ...
                                    {/* Show valid trains */}
                                    {validTrains.length === 0 && errors.length === 0 ? (
                                        <p className="text-gray-500 italic">{t.noTrainsAvailable}</p>
                                    ) : validTrains.length > 0 ? (
                                        <div className="space-y-4">
                                            {aggregateByTrain(validTrains).map((train, index) => (
